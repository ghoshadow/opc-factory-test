...
import { Check, X, AlertTriangle, Clock, Shield, FileText, GitPullRequest, Search } from "lucide-react"
import { cn } from "@/lib/utils"
import type { PlanReviewItem, DesignReviewItem, CodingOpsResponse } from "@/types/factory"
...
const severityConfig = {
  ...
  major: { bg: "bg-orange-100 dark:bg-orange-950/30", text: "text-orange-700 dark:text-orange-400", label: "Major" },
  minor: { bg: "bg-amber-100 dark:bg-amber-950/30", text: "text-amber-700 dark:text-amber-400", label: "Minor" },
...
function ReviewItemRow({ item }: { item: PlanReviewItem | DesignReviewItem }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-card px-4 py-3">
      ...
      <span className={cn("inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium shrink-0", item.status === "pass" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400" : item.status === "fail" ? "bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-400" : item.status === "warning" ? "bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400" : "bg-muted text-muted-foreground")}>
        {cfg.label}
      </span>
    </div>
  )
    ...
    <div className="rounded-xl border bg-card shadow-sm p-6 space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground tabular-nums">
            {data.tocoReport.metrics.filesChanged} 文件 · {data.tocoReport.metrics.changedLines} 行变更
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-3 text-center">
            ...
            <p className="text-lg font-bold tabular-nums text-blue-600">{data.tocoReport.metrics.changedLines}</p>
          </div>
          <div className="rounded-lg border border-border bg-muted/30 p-3 text-center">
          ...
          <div className={cn(
            ...
            <p className={cn("text-lg font-bold tabular-nums", data.tocoReport.metrics.complexityDelta > 0 ? "text-red-600" : "text-emerald-600")}>
              {data.tocoReport.metrics.complexityDelta > 0 ? "+" : ""}{data.tocoReport.metrics.complexityDelta}
            </p>
          </div>
        </div>
        ...
        {data.tocoReport.findings.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-muted-foreground">发现项 ({data.tocoReport.findings.length})</p>
            {data.tocoReport.findings.map((finding) => {
              const sev = severityConfig[finding.severity]
              return (
                <div key={finding.id} className="flex items-start gap-3 rounded-lg border border-border bg-card px-4 py-3">
                  <span className={cn("inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold shrink-0 mt-0.5", sev.bg, sev.text)}>
                    {sev.label}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{finding.title}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{finding.description}</p>
                    <p className="text-[10px] text-muted-foreground/60 mt-1 font-mono">{finding.lineRef}</p>
                  </div>
                  <span className="text-[10px] text-muted-foreground shrink-0">{finding.category}</span>
                </div>
              )
            })}
          </div>
