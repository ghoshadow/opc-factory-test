...
const fetcher = (url: string): Promise<IntakeResponse> => fetch(url).then((res) => res.json());
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "标题",
        sortable: true,
        render: (item: IntakeItem) => <span className="font-medium">{item.title}</span>,
      },
      {
        key: "priority",
        header: "优先级",
    ...
    return (
      <>
        <FilterToolbar
          ...
          onStatusChange={setFilterStatus}
        />
        <EmptyState icon={FileText} title="暂无入厂需求" description="点击新建需求开始" />
      </>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {items.length === 0 && hasActiveFilters ? (
        <EmptyState icon={Filter} title="无匹配需求" description="尝试调整筛选条件" />
      ) : (
        <DataTable
          columns={columns}
          data={items}
                ...
                <div className="flex flex-wrap gap-2">
                  <span
                    className={cn(
                      ...
                      priorityStyle[selectedItem.priority],
                    )}
                  >
                  ...
                  <span
                    className={cn(
                      "inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium",
                      statusStyle[selectedItem.status],
                    )}
                  >
...
function FilterToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {total !== undefined && (
        <span className="ml-auto text-xs text-muted-foreground tabular-nums">共 {total} 项</span>
      )}
    </div>
  );
}
