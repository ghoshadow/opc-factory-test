...
import type { PipelineNodeStatus, PipelineResponse, PipelineStageNode } from "@/types/factory";
...
const pipelineNodes: PipelineStageNode[] = [
  ...
  },
  {
    ...
    details: [
      "选择器生成: CSS/XPath/文本/角色选择器",
      "视觉定位: 截图比对、坐标定位",
      "API 路径匹配: REST/GraphQL 端点识别",
      "数据锚点: 数据库记录、缓存键定位",
    ],
  },
  {
    ...
    details: [
      ...
      "输入: 文本输入/文件上传/富文本编辑",
      "手势: 拖拽/滑动/缩放",
      "HTTP 调用: GET/POST/PUT/DELETE + headers",
      "数据 seed: 测试数据准备与注入",
    ],
  },
  {
  ...
  },
  {
    ...
    details: [
      "驱动: Playwright/Selenium/API Client",
      "回放: 录屏、截图、日志回放",
      "断言: HTTP 状态码、页面元素、数据库状态",
      "产物采集: 截图、视频、HAR、trace 文件",
    ],
  },
  {
    id: "bug-triage",
    ...
    details: [
      "P0-P3 分诊: 阻塞/严重/一般/轻微",
      "回溯链: Bug → 测试用例 → AC → Spec",
      "相似聚类: 文本相似度 + 堆栈聚类",
      "Owner 路由: 模块 → 负责人自动分配",
    ],
  },
  {
    ...
    details: [
      "AC 覆盖率: 每个 AC 至少对应一条用例",
      "Flaky 检测: 重复执行 3 次确认稳定性",
      "证据完整性: 截图/视频/日志齐全",
      "性能基线: 响应时间/吞吐量在基线 ±20% 内",
    ],
  },
  {
