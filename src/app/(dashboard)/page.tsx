...
const fetcher = (url: string): Promise<MetricsResponse> => fetch(url).then((res) => res.json());
...
export default function DashboardPage() {
  ...
  const systemHealthLabel =
    kpi.systemHealth >= 75 ? "良好" : kpi.systemHealth >= 50 ? "一般" : "需关注";
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">4 条产线的实时状态与关键指标</p>
      </div>

      {/* KPI cards - 7 columns */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        <MetricCard label="在制总量" value={kpi.totalWip} unit="项" icon={Factory} />
        <MetricCard label="已完成总量" value={kpi.totalCompleted} unit="项" icon={CheckCircle2} />
        <MetricCard
          label="需关注产线"
        ...
        />
        <MetricCard label="活跃需求" value={kpi.activeRequirements} unit="项" icon={FileText} />
        <MetricCard label="待评审" value={kpi.pendingReviews} unit="项" icon={GitPullRequest} />
        <MetricCard label="今日部署" value={kpi.todayDeployments} unit="次" icon={Rocket} />
        <MetricCard
          label="系统健康度"
